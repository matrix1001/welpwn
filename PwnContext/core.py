...
class PwnContext(object):
    ...
    @_validator
    def custom_gdbscript(self, custom_gdbscript):
        """str: Custom gdb scripts to execute after the autogenerated commands
        """
        if not custom_gdbscript:
            return None
        assert type(custom_gdbscript) == str
        return custom_gdbscript
    ...
    def gdbscript(self):
        ...
        for bp in breakpoints:
            result += 'b *{:#x}\n'.format(bp + prog_base)
        if self.custom_gdbscript:
            result += self.custom_gdbscript
        print("Generated gdbscript: " + repr(result))
        return result
